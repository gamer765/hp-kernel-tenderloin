	...
	for (i = 0; i < USB_STS_RETRIES && status == -ETIMEDOUT; i++) {
		for(j=0; j<3; j++){
			...
			if (status==4)
			{
				...
				printk("trying to get port status again *****status%x portstatus=%x\n",
					status, data->wPortStatus);
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	/* Most hubs have proper suspend/resume support */
	...
	if (status == 0) {
		...
		for(i=0; i<3; i++){
			...
			if (status > 0)
				...
				printk("trying to get device status again****status=%u\n", status);
			mdelay(100);
		}
		if (status >= 0)
